...
def make_mult_png(ra, dec, surveys, bands,
                  fovs, apertures, square_sizes, image_path, n_col= 3,
                  verbosity=0):
    ...
    for idx, survey in enumerate(surveys):
        ...
        if survey == 'vhsdr6' :
            coord_name = ut.coord_to_vhsname(np.array([ra]),
                                             np.array([dec]),
                                             epoch="J")
        else :
        ...
        filenames_available = glob.glob(image_path + '/' + coord_name[0] +
                                        "_" +
                                        survey
                                        + "_" + band + "*.fits")
        ...
        if file_found == True:
            ...
            if verbosity > 0:
                print("File associated to {} in {} {} not found".format(
                    coord_name, survey, band))
        ...
        if file_found:
            ...
            simg = axs.imshow(img_stamp, origin='lower', cmap='gray',
                             vmin=z1, vmax=z2)
